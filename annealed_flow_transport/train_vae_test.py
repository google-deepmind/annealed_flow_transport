...
class TestKLDivergence(parameterized.TestCase):
  ...
  def test_kl_divergence(self):
    ...
    test_kl = train_vae.kl_divergence_standard_gaussian(mean, std)
    _assert_equal_vec(self, test_kl, 0.)

    mean_b = jnp.arange(4)
    std_b = jnp.array([1.3, 1.7, 1.8, 2.0])

    test_kl_b = train_vae.kl_divergence_standard_gaussian(mean_b, std_b)
  ...
  def test_batch_kl_divergence(self):
    ...
    test_mean_kl = train_vae.batch_kl_divergence_standard_gaussian(means,
                                                                   stds)
...
class TestBinaryCrossEntropy(parameterized.TestCase):
  ...
  def test_binary_cross_entropy(self):
    ...
    test_loss = train_vae.binary_cross_entropy_from_logits(trial_logits,
                                                           trial_labels)
